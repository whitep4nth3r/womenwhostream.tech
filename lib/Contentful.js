...
export default class Contentful {
  ...
  static async getTwitchUserName(entryid) {
    /*
    Query using `preview: true` to retrieve unpublished entries.
    This is support pre-populating twitch data when adding (create) new streamers,
    and un-registering the twitch event sub for unpublished entries.
    */
    const query = `{
    ...
    const usernames = await this.call(query, { preview: true });
    return usernames.data.streamerCollection.items[0].twitchUsername;
  }

  static async updateStreamerByTwitchUsername(username, twitchData, streamData, vodData, tagData, publish = true) {
    const query = `{
    ...
    await this.updateStreamerById(id, twitchData, streamData, vodData, tagData, publish);
  }

  static async updateStreamerById(id, twitchData, streamData, vodData, tagData, publish = true) {
    ...
    await client
      ...
      .then((entry) => console.log(`Entry ${entry.sys.id} - ${entry.fields.twitchUsername["en-US"]} updated.`))
      .catch((error) => {
  ...
  static async call(query, options = { preview: false }) {
    const accessToken = options.preview ? process.env.CTFL_PREVIEW_ACCESS_TOKEN : process.env.CTFL_ACCESS_TOKEN;
