...
export const createSubscription = async (authToken, type, userid, callbackurl) => {
  const body = {
    type: type,
    version: "1",
    condition: {},
  ...
  const response = await fetch("https://api.twitch.tv/helix/eventsub/subscriptions", request);
  return response;
};
...
export const getUsersByLogin = async (login, authToken) => {
  ...
  const response = await fetch(`https://api.twitch.tv/helix/users?login=${login}`, request);
...
export const getUsersById = async (id, authToken) => {
  ...
  const response = await fetch(`https://api.twitch.tv/helix/users?id=${id}`, request);
  return await response.json();
};
...
export const getStream = async (id, authToken) => {
  ...
  const response = await fetch(`https://api.twitch.tv/helix/streams?user_id=${id}`, request);
  return await response.json();
};
...
export const getVideo = async (id, authToken) => {
  ...
  const response = await fetch(`https://api.twitch.tv/helix/videos?user_id=${id}&first=1`, request);
  return await response.json();
};
...
export const verifyMessage = (messageid, timestamp, body, signature) => {
  const hmac_message = messageid + timestamp + Buffer.from(JSON.stringify(body), "utf8");
  const newsignature = createHmac("sha256", process.env.TWITCH_EVENTSUB_SECRET)
    .update(hmac_message)
    .digest("hex");
  if (signature !== `sha256=${newsignature}`) {
    return false;
  }
  return true;
};
